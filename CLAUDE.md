...
- **P0** — CSV ingestion: `scripts/import_moon_requirements.py` parses the community "Moon Ability Requirements" CSV (repo root, Devon's authored work, safe to commit) → `data/moon_requirements.json` (775 CSV moons; **all 775 now matched to an AP location** — the 213 the curated upstream pool originally omitted were promoted to AP checks 2026-06-19, see "Moon coverage" below) + `data/subareas.json` (131 subareas). Vocabulary normalised to ability enums.
...
- **P2** — capturesanity + fixed starters. (HISTORICAL NOTE: P2 originally *removed* capturesanity, but it was later re-introduced as a live `capturesanity` `DefaultOnToggle` option — see `hooks/Options.py`. `before_is_category_enabled` must NOT disable "Capture" — it would zero out the capture *items* that share the category; capture *locations* are dropped via `before_is_location_enabled` instead, see `hooks/Helpers.py`.) `_precollect_starting_captures()` gives THREE fixed starters — Frog + Chain Chomp + **Broode's Chain Chomp** (added for the Cascade scenario-gating fix) — plus 1 random each seed (`FIXED_STARTER_CAPTURES`, `hooks/World.py`). Spark pylon + Bowser are NO LONGER precollected — they ride the pool as progression gating items.
...
**Moon coverage — the 213 omitted moons promoted to AP checks (2026-06-19).** The curated upstream pool exposed only 562 of the 775 `moon_requirements.json` moons; the other 213 (Koopa Freerunning Cups, "Peach in the X Kingdom", Hint Art, Hat-and-Seek, Taking Notes, Caught Hopping, Timer Challenges, ~140 plain overworld moons) are now AP locations. `data/locations.json` is **818** locations. Generated by `scripts/add_missing_moon_locations.py` (committed-data-only, idempotent): derives `<Kingdom>: <suffix>` names, back-fills `location_name` in `moon_requirements.json`, tags each into an opt-out cluster category (new `DefaultOnToggle`s: `include_{cup,peach,taking_notes,timer_challenge,hat_and_seek,caught_hopping,extra}_moons`), grows each kingdom's `Power Moon` item count + the matching `*MoonCount` Range ceiling. **NO location `progression` flag** on the new moons — that flag is the 38-moon Talkatoo% scenario-advancer audit (`tests/test_progression_moons.py`), not a progression-kingdom marker. The importer was **re-run this session** so `subareas.json` lists the 16 new subarea moons (closes the only subarea-gate gap). ⚠ **Mandatory before generate/play (on the machine WITH romfs data):** `compile_moon_logic.py` (fills the 213's `requires` + peace/scenario gating — they currently ship `requires:""`) → `sync_shine_table.py` (verify `// Count:` grew ~213; a name that doesn't byte-match `shine_map.json` gets NO row and is uncollectable in-game) → `install_apworld.py` → `Generate.py`. **NEVER run `compile_moon_logic.py` without `shine_map.json`/`world_scenarios.json` present** — it degrades to rock-only and WIPES the scenario gates already compiled into the existing 562 moons. Full detail: [docs/logic-and-entrance-status.md](docs/logic-and-entrance-status.md) §3.
...
**P7 — entrance shuffle: LIVE + VALIDATED IN-GAME (2026-06-19).** `kEntranceRemapApply` in `switch-mod/src/hooks/EntranceShuffleHook.cpp` is now `true` — the remap actually rewrites `mChangeStageName`/`mChangeStageId` in the ChangeStageInfo (was preview-only). Coupled bijection ships in slot_data over the wire, so the flip is **switch-mod-only** (no apworld rebuild / re-seed). Entry rows keyed on `dest` (inbound interior), exit rows keyed on `cur` (`getCurrentStageName`); both exit classes handled (`returnPrevStage` pops to origin already-correct; `changeNextStage` gets the exit-by-cur rewrite). `dest==cur` guard skips moon-rock same-stage reloads. Apply-mode walk passed every case: doors, pipes, moon pipes, multi-exit subareas (both exit pipes collapse to origin), forward + return, with story areas (Sky Garden Tower) and boss/cutscene warps (Spewart) correctly untouched. Logs + table: [docs/devon-p7-entrance-testing-results.md](docs/devon-p7-entrance-testing-results.md) (APPLY-MODE section); memory [[entrance-shuffle-live-validated]]. **Open follow-ups:** (a) Rules.py reachability watch — moon-pipe moons reached via shuffled origin (Devon's, when applicable); (b) kingdom-order gate exposes not-yet-reachable kingdoms as selectable, only the BACKSTOP enforces order — orthogonal, see [[kingdom-order-gate-premature-destinations]]. **Decoupled flight-economy fix (2026-07-12):** decoupled seeds were beating the goal in ~8 spheres because the free "K Arrival → K" chain-presence edge let on-foot arrival discount the cumulative `{KingdomMoons}` flight economy (`reach(Moon)` collapsed to ~one kingdom's gate). Fixed by `hooks/World.py::_apply_decoupled_flight_economy` — ANDs a recursive `flight_reach(source)` predicate onto every inter-kingdom flight edge (decoupled-only, `add_rule` not `set_rule`, Pokino + "K Arrival" exempt); restores the full 124-moon (rolled-sum) cost. Audit + plan: [docs/handoff-decoupled-flight-economy-fix.md](docs/handoff-decoupled-flight-economy-fix.md); D1 erratum in [docs/design-decoupled-kingdom-order.md](docs/design-decoupled-kingdom-order.md). **Mushroom route + directed roller (2026-07-17, D10):** decoupled matched pairs are TWO-WAY portals (exits chain — the "sphere-1 Mushroom leak" was logic faithfully following real portal routes, not false reachability); Devon ruled exit-portals ARE the MK-overworld route and early MK moons are fine. `port_matching.py` now treats `NON_ROOT_KINGDOMS` (Mushroom — post-game, not flight-reachable pre-goal) stages as non-roots and phase 1 grows a DIRECTED frontier (`_directed_model` / `directed_full_interior_strands`), certifying at roll time that every subarea's FULL interior is reachable (no more entry-mouth deadlock pairs) and MK has a real portal route. Still owed: in-game walk of a portal landing in pre-credits PeachWorldHomeStage (D9 moon-spawn probe). See D10 + the resolution banner in [docs/handoff-decoupled-mushroom-overworld-reachability.md](docs/handoff-decoupled-mushroom-overworld-reachability.md). **MK tower shuffle (2026-07-18):** the 6 "… Boss Re-fight" pool records are re-pointed at the TOWER stages (`PeachWorldPicture*Stage` — REAL two-way stages, romfs-verified; the old ZONE_STAGE_ALIAS "zone" classification was wrong and is removed), so shuffled doors can lead into a refight tower in both modes; the painting → `RevengeBoss*Stage` → Multi-Moon-return-to-tower loop stays vanilla (the MM's ShineGrand actor returns to `tower#PictureBoss*`; protected by per-port `from_id` entry rows in `compile_stage_remaps` + the always-`from_id` decoupled rows). Rematch checks key on shuffled tower access + the boss capture. apworld-only, needs reseed, NO switch-mod rebuild. See the resolution banner + Devon's in-game list in [docs/handoff-mk-tower-shuffle.md](docs/handoff-mk-tower-shuffle.md).
...
**Devon-fork features (2026-06-11/12):** `randomize_kingdom_gates` (sum-preserving ±5 rolls, total pinned to vanilla 124; rolled gates flow slot_data → `kingdom_gates` wire msg → `ApState::kingdom_gate[]` → `UnlockShineNumHook` so the in-game Odyssey cost matches logic), `multi_moon_shuffle` (default on; **21 MM items ↔ 21 `multi_moon`-tagged locations** as of 2026-07-05 — the 14 story-boss MMs plus the 6 Mushroom Kingdom re-fights + 1 Dark Side arrival added by the re-fight/Dark Side bundle feature; PM-first demotion variant, one Metro MM dropped under the festival goal — "A Traditional Festival!" is the festival victory location and holds no item, and under festival the 7 new post-Metro MMs are dropped too. **Re-fight/Dark Side bonus side-grants:** each Mushroom Kingdom Multi-Moon unlocks 3 bonus captures, the Dark Side Multi-Moon 3 bonus abilities — "bonus on top" of the capturesanity/abilitysanity pool (dupes→coins). Rolled random-from-locked at gen (`World._roll_mm_bonus_grants`, `world.random`), shipped via slot_data `mm_bonus_captures` (18, ordered) + `mm_bonus_abilities` (3); the client (`context._process_received_items`) consumes the 18-list in ordered chunks of 3 as each same-named Mushroom MM arrives and folds the 3 abilities into the ability snapshot. apworld+client only — NO switch-mod rebuild, NO `sync_shine_table`. See [docs/handoff-refight-multi-moons.md](docs/handoff-refight-multi-moons.md) and [[refight-multimoon-bundles]]. **Bonus grants MUST be replayable from derived state, not from the item mirror** (fixed 2026-07-17): the Switch wipes `captures_unlocked` on EVERY save load (`SaveLoadHook.cpp`) and rebuilds it from the HELLO replay, which walks `received_items` — where a bonus grant has no entry (its Multi-Moon is there but is Moon-skipped by M6 phase D). So the live `send_item` at grant time is not durable delivery: `BridgeState.bonus_captures` + `switch_server.push_bonus_captures()` re-ship on every HELLO. Bonus ABILITIES never had this bug — they ride `push_ability_state`'s derived full-count snapshot. Anything else that synthesizes a Switch-side unlock without a real AP item needs the same treatment. See [docs/handoff-mm-bonus-capture-enforcement.md](docs/handoff-mm-bonus-capture-enforcement.md)), Ruined moons promoted to progression + demotion-exempt (gated-kingdom moons MUST be progression or the reachability sweep can't satisfy their gate — guard test in test_randomize_kingdom_gates.py), and **peace-gated Moon Rocks** (`MoonRockHook`: openable after per-kingdom story completion instead of game clear; NEVER force while the moon-rock scenario is active — the vanilla rock-open is a scenario-jump stage reload whose re-init must take the wreckage/commit branch; mid-story forcing without the peace gate skips kingdom bosses and strands story-MM checks). Cap-peace-from-start experiment concluded OFF — scenario numbers are recomputed from quest state at every load; see MoonRockHook.cpp header. `install_apworld.py --out` exists so the bundling tests never clobber the real installed zip (zipimport TOC staleness presented as 'bad local file header').

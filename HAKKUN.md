...
| 3b (residual: ApClient.cpp port) | `7703757` | ApClient's 1124-line networking core ported. `nn::socket::*` → `hk::socket::Socket::instance()->{socket,connect,send,recv,close,poll,setSockOpt}`; `nn::nifm::*` preserved + resolved via new `switch-mod-hk/syms/nn/nifm.sym` (mangled lengths 10/27/18 — the original residual-row guess at 28/19 was off by one); `nn::os::CreateThread` → in-place `hk::os::Thread` ctor with 64 KiB page-aligned static stack; manual sockaddr workaround replaced by runtime `hk::socket::SocketAddrIpv4::parse(host, port)`; `Select+fd_set` → `PollFd[1]+poll`. Two Hakkun-side template quirks worked around at call sites (not yet upstream-patched): `Socket::connect`'s unused 2nd template parameter requires explicit args; templated `setSockOpt(fd, lvl, opt, const T&)` fails to compile so use the Span<const u8> overload. CMakeLists.txt picked up `add_compile_definitions` mirroring switch-mod (SMO_*_VERSION_STRING + BRIDGE_*). subsdk8 grew to ~115 KiB; sail resolves 51 SMO symbols (48 game + 3 nifm). Build clean; runtime validation deferred. |
| 5 | _TBD_ | All three validation gates pass (loopback, Ryujinx manual, real-Switch FW22). **Prerequisite:** disable the production `smo-archipelago` mod in Ryujinx before booting — both subsdks install hooks at the same SMO symbol addresses, so they'd collide if loaded together. Once disabled, the new `smo-archipelago-hk` subsdk8 mod runs alone. |

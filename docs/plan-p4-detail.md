...
| Ledge Grab | Ledge grab / ceiling swing pole | **NOT gated in-game — LOGIC-ONLY.** `PlayerJudgeGrabCeil::judge` was hooked, but it also catches the horizontal **ceiling swing bar** (`PoleGrabCeil` / `PoleGrabCeilExStage`): Mario latches via a sensor message that bypasses the judge, then his swing/jump-off does NOT flow back through this judge (an `exeGrabCeil` "allow-while-grabbing" beacon was built + tested in-game 2026-06-25 and STILL soft-locked — the exit is a different nerve), so suppressing the judge stranded him mid-jump on the bar. There are only 3 pole moons, so per Devon the in-game gate was **removed** (vanilla pole behavior, no soft-lock). Ledge Grab is **auto-granted with Wall Slide**, so it's enforced in **generation logic** as `\|Wall Slide\|` (NOT `\|Ledge Grab\|`) on *Metro: Hanging from a High-Rise*, *Metro: Vaulting Up a High-Rise* (already had it), *Wooded: Swing Around Secret Flower Field* — locations.json + moon_requirements.json `other_required:["wall_slide"]`. | ≥1 (logic, via Wall Slide) | ✅ logic-only |
| Climb | Pole/net climb | `PlayerJudgePoleClimb::judge` — clean entry block (Mario can't grab a vertical pole/net without Climb; no soft-lock, unlike the GrabCeil ceiling pole). Load-bearing in generation logic (dozens of `\|Climb\|` requires). | ≥1 | ✅ |
